...
flowchart TD
    ...
    subgraph SG_PARSE1["📄 Parsing  (pipeline/parser.py)"]
    ...
    subgraph SG_EMBED1["🔢 Vector Indexing  (pipeline/embedder.py)"]
    ...
    QUERY1([❓ User Query q]) --> R1

    subgraph SG_RETRIEVE1["🔍 Dense Retrieval & Reranking  (pipeline/retriever.py)"]
    ...
    subgraph SG_GEN1["🤖 Generation  (pipeline/generator.py)"]
    ...
    style SG_PARSE1 fill:#dbeafe,stroke:#3b82f6,color:#1e3a5f
    style SG_EMBED1 fill:#fef3c7,stroke:#d97706,color:#451a03
    style SG_RETRIEVE1 fill:#d1fae5,stroke:#059669,color:#064e3b
    style SG_GEN1 fill:#fce7f3,stroke:#db2777,color:#500724
```
...
flowchart TD
    ...
    subgraph SG_PARSE2["📄 4-Level Hierarchical Parsing  (pipeline/parser.py)"]
    ...
    subgraph SG_EMBED2["🔢 Vector Storage  (pipeline/embedder.py)"]
        EM["BAAI/bge-m3  (dim=1024)\nQdrant collection: 'hier_finance_rag'\nSections, Text, Tables, and Cells indexed separately"]
    end

    GRF --> EM

    QUERY2([❓ User Query q]) --> RT1

    subgraph SG_ROUTE2["🧭 Two-Stage Intent Router  (pipeline/router.py)"]
    ...
    subgraph SG_RETRIEVE2["🔍 Three-Level Hybrid Retrieval  (pipeline/retriever.py)"]
    ...
    subgraph SG_GEN2["⚙️ Symbolic-Neural Fusion Generator  (pipeline/generator.py)"]
        GENC{Classified\nIntent?}
    ...
    subgraph SG_ATTR2["📎 Grounded Attribution  (pipeline/attribution.py)"]
    ...
    style SG_PARSE2 fill:#dbeafe,stroke:#3b82f6,color:#1e3a5f
    style SG_EMBED2 fill:#fef3c7,stroke:#d97706,color:#451a03
    style SG_ROUTE2 fill:#ede9fe,stroke:#7c3aed,color:#2e1065
    style SG_RETRIEVE2 fill:#d1fae5,stroke:#059669,color:#064e3b
    style SG_GEN2 fill:#fce7f3,stroke:#db2777,color:#500724
    style SG_ATTR2 fill:#fee2e2,stroke:#dc2626,color:#450a0a
```
...
flowchart TD
    ...
    subgraph SG_VPARSE3["🖼️ Vision Table Parser  (pipeline/parser.py)"]
    ...
    subgraph SG_POST3["🔁 Post-Processing & Normalization"]
    ...
    subgraph SG_EMBED3["🔢 Vector Storage  (pipeline/embedder.py)"]
        EM3["BAAI/bge-m3 (dim=1024)\nQdrant collection: 'tatr_finance_rag'"]
    end

    PP --> EM3

    QUERY3([❓ User Query q]) --> ROUTER_TATR

    subgraph SG_ROUTE3["🧭 Intent Router  (pipeline/router.py)"]
        ROUTER_TATR["Two-Stage Classifier:\n1. Keyword rule scoring\n2. Single-token LLM fallback\n→ Numerical / Lookup / Comparison / Summarization"]
    end

    ROUTER_TATR --> RL1

    subgraph SG_RETRIEVE3["🔍 Structure-Aware Retrieval  (pipeline/retriever.py)"]
    ...
    EM3 -.->|Vector Lookup| RL1
    RL3 --> GENV

    subgraph SG_GEN3["⚙️ Generator & Two-Pass Extraction  (pipeline/generator.py)"]
        GENV{Intent?}
    ...
    subgraph SG_ATTR3["📎 Attribution  (pipeline/attribution.py)"]
        AT2["Supporting Cells · Supporting Sentences\nConfidence Calibration · Attribution Mapping"]
    end

    AT2 --> OUT3([✅ Final Answer + Verified Evidence])

    style SG_VPARSE3 fill:#dbeafe,stroke:#3b82f6,color:#1e3a5f
    style SG_POST3 fill:#f3f4f6,stroke:#6b7280,color:#111827
    style SG_EMBED3 fill:#fef3c7,stroke:#d97706,color:#451a03
    style SG_ROUTE3 fill:#ede9fe,stroke:#7c3aed,color:#2e1065
    style SG_RETRIEVE3 fill:#d1fae5,stroke:#059669,color:#064e3b
    style SG_GEN3 fill:#fce7f3,stroke:#db2777,color:#500724
    style SG_ATTR3 fill:#fee2e2,stroke:#dc2626,color:#450a0a
```
