...
def execution_accuracy(predicted: Optional[str], expected,
    ...
    if pred_num is not None and exp_num is not None:
        ...
        if pred_has_pct and not exp_has_pct and abs(exp_num) < 1:
            ...
            exp_num /= 100.0
        # Change 17: Wider implicit scale reconciliation.
        # Handles cases where LLM drops the % sign and the values differ by ~100x.
        elif not pred_has_pct and not exp_has_pct:
            if abs(exp_num) > 0:
                ratio = abs(pred_num) / abs(exp_num)
                # pred is ~100x expected (e.g. exp=0.0782, pred=7.82 or 0.78)
                if 0.5 < ratio < 150 and abs(exp_num) < 1 and abs(pred_num) >= 0.1:
                    rescaled = pred_num / 100.0
                    rel_rescaled = abs(rescaled - exp_num) / max(abs(exp_num), 1e-9)
                    if rel_rescaled <= 0.15:   # within 15% after rescaling
                        pred_num = rescaled
                # pred is ~1000x expected (billion vs million confusion)
                elif 800 < ratio < 1200:
                    rescaled = pred_num / 1000.0
                    rel_rescaled = abs(rescaled - exp_num) / max(abs(exp_num), 1e-9)
                    if rel_rescaled <= 0.15:
                        pred_num = rescaled
...
def extract_predicted_value(answer_text: str, is_symbolic: bool,
                            intent: str = "") -> Optional[str]:
    """Extract the final predicted value from a generated answer string.

    Search order:
        0. Direct Answer line (Comparison queries: 'Direct Answer: <text>').
    ...
    # 0. Comparison: extract the 'Direct Answer: ...' line verbatim.
    #    This is a categorical answer (e.g. 'improving', 'Google Cloud')
    #    not a number, so we return it before any numeric extraction.
    if intent == "Comparison":
        da_match = re.search(r"Direct\s+Answer:\s*(.+?)(?:\n|$)", text, re.IGNORECASE)
        if da_match:
            return da_match.group(1).strip()
    ...
    # Change 19: Pattern captures full decimals like 1.7239 at end-of-string
    # by using a non-greedy decimal group and boundary anchor.
    if is_symbolic:
        matches = re.findall(r"=\s*\**(-?[\d,]+(?:\.\d+)?%?)\**(?=\s|\*|$|\n)", text)
        if not matches:
            # Fallback: looser pattern without lookahead
    ...
    if is_symbolic:
        ...
        # Pattern requires digits after decimal point to avoid capturing
        # trailing dots (e.g. '1.' from truncated output).
...
def run_evaluation(
    ...
    for i, item in enumerate(dataset):
        ...
        try:
            ...
            is_sym    = intent == "Numerical"  # Comparison uses text extraction, not symbolic
            predicted = extract_predicted_value(answer, is_symbolic=is_sym, intent=intent)
